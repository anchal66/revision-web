...
import { ArrowRight, BookOpen, Code, Layers, Sparkles, Server, Cloud } from 'lucide-react';
...
export function TopicBrowser({ topics }: TopicBrowserProps) {
    ...
    const groupedTopics = useMemo(() => {
        const groups: Record<string, Topic[]> = {
            ...
            'DSA': [],
            'Cloud & Architecture': [],
            'System Design': [] // Placeholder if needed in future
        };

        filteredTopics.forEach(topic => {
            const cat = topic.category || 'DSA'; // Fallback
    ...
    return (
        <div className="space-y-12">
            {/* Controls */}
            <div className="flex flex-col sm:flex-row gap-4 items-center justify-between border-b pb-6 sticky top-0 bg-background/95 backdrop-blur-sm z-30 pt-4">
            ...
            {filteredTopics.length === 0 ? (
                ...
                <div className="space-y-16">
                    {/* Maps through groups (e.g., Java & Spring, DSA) */}
                    {groupedTopics.map(([category, items]) => (
                        <section key={category} className="space-y-6">
                            <div className="flex items-center gap-3">
                                <div className="p-2 rounded-lg bg-primary/10 text-primary">
                                    {category === 'DSA' && <Code className="w-5 h-5" />}
                                    {category === 'Java & Spring' && <Server className="w-5 h-5" />}
                                    {category === 'Cloud & Architecture' && <Cloud className="w-5 h-5" />}
                                </div>
                            ...
                            >
                                {items.map((topic) => (
                                    <TopicCard key={topic.slug} topic={topic} viewMode={viewMode} />
                                ))}
                            </div>
                        </section>
                    ))}
...
// Sub-component for individual cards to keep the main file clean
function TopicCard({ topic, viewMode }: { topic: Topic, viewMode: 'grid' | 'list' }) {
    return (
        <Link href={`/${topic.slug}`} className="group h-full">
